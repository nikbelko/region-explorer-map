...
const RegionInfoPanel = ({ selectedRegion, regionStats, onClearRegion }: RegionInfoPanelProps) => {
  ...
  if (!selectedRegion) {
    return (
      <div className="rounded-lg border border-border bg-card p-4">
        ...
        <p className="text-sm text-muted-foreground italic">Выберите регион на карте</p>
      </div>
    );
  }
  ...
  const top3Share = totalPoints > 0 ? Math.round(top3Brands.reduce((s, b) => s + b.count, 0) / totalPoints * 100) : 0;
  const totalDynamics = regionStats?.brands.reduce((sum, b) => sum + getBrandDynamics(selectedRegion, b.brand, period), 0) ?? 0;

  return (
    <div className="rounded-lg border border-border bg-card p-4">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Инфопанель региона</h3>
        <button onClick={onClearRegion} className="p-1 rounded-md hover:bg-secondary text-muted-foreground hover:text-foreground transition-colors" title="Сбросить выбор региона">
      ...
      <div className="space-y-4">
        ...
        <div className="text-xs text-muted-foreground italic space-y-0.5">
          {population && <p>Население: ~{population} млн <span className="ml-1 text-[10px] opacity-60">(эксп.)</span></p>}
          {area && <p>Площадь: {area.toLocaleString()} км² <span className="ml-1 text-[10px] opacity-60">(эксп.)</span></p>}
          {populationDensity && <p>Плотность: {populationDensity.toLocaleString()} чел/км² <span className="ml-1 text-[10px] opacity-60">(эксп.)</span></p>}
        </div>
        ...
        <div className="grid grid-cols-1 gap-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="rounded-md bg-secondary p-2.5 group/density relative">
              ...
              <p className="text-base font-bold text-foreground mt-0.5">{concentrationIndex !== null ? concentrationIndex : "—"}</p>
            </div>
            <div className="rounded-md bg-secondary p-2.5">
              <p className="text-[10px] text-muted-foreground leading-tight">Топ-3 доля рынка</p>
              <p className="text-base font-bold text-foreground mt-0.5">{totalPoints > 0 ? `${top3Share}%` : "—"}</p>
            </div>
          </div>
          ...
          <div className="rounded-md bg-secondary p-2.5">
            ...
            <p className={`text-base font-bold mt-0.5 ${totalDynamics >= 0 ? "text-green-600" : "text-red-500"}`}>
        ...
        {regionStats && regionStats.totalPoints > 0 ? (
          <div className="space-y-2">
            <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">По брендам</p>
                ...
                <div key={b.brand} className="space-y-1">
                  <div className="flex items-center justify-between py-1 border-b border-border/50 last:border-0">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-[10px] font-medium ${dynamics >= 0 ? "text-green-600" : "text-red-500"}`}>
                        {dynamics >= 0 ? "+" : ""}{dynamics}
                      </span>
          ...
          <p className="text-sm text-muted-foreground italic">Нет точек, соответствующих критериям</p>
        )}
      </div>
    </div>
  );
