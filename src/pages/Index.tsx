...
/* Nav button with tooltip rendered in a portal-like fixed position */
const NavBtn = ({
  ...
  <div className="relative group" style={{ isolation: "isolate" }}>
    ...
    {/* Tooltip — fixed z-index above Leaflet (z-[1001]+) */}
    <span
...
const Index = () => {
  ...
  return (
    ...
    >
      {/* ── Navbar — z-[9998] so tooltips at z-[9999] win ── */}
      <nav
      ...
      <aside
        ...
        style={{ width: filterPanelOpen ? "260px" : "0px", zIndex: 10 }}
      >
      ...
      {/* ── Getplace-style collapse toggle ──
          Small dark pill button flush against the sidebar edge */}
      <button
        ...
        onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.background = "#b0b7c3"; }}
        onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.background = "#d1d5db"; }}
      >
        {filterPanelOpen
      ...
      {/* ── Right region detail ── */}
      {selectedRegion && (
        <aside className="w-[272px] flex-shrink-0 border-l border-gray-200 bg-white flex flex-col overflow-y-auto" style={{ zIndex: 10 }}>
          <RegionInfoPanel
            selectedRegion={selectedRegion}
            regionStats={regionStats}
